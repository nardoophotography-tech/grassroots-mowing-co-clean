...
export const Booking = () => {
  ...
  // Manual address entry (Google Maps removed from the booking flow).
  const [manualAddress, setManualAddress] = React.useState('');
  const [accessNotes, setAccessNotes] = React.useState('');
  React.useEffect(() => {
    // Re-populate the address field if the user navigates back to this step.
    if (watchedValues.location?.address && !manualAddress) {
      setManualAddress(watchedValues.location.address);
    }
  }, [watchedValues.location?.address]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const onSubmit = async (data: BookingFormValues) => {
    ...
    try {
      ...
      if (!snapshot.isQuoteRequired && (data.clientType === 'one_off')) {
         // Auto-confirm for one-off (payment on arrival) — durable DB-backed confirmation
         navigate(`/booking-success?jobId=${jobId}`);
      } else if (!snapshot.isQuoteRequired && data.clientType === 'returning') {
        setStep(6);
      } else {
        // Quote required — durable DB-backed confirmation
        navigate(`/booking-success?jobId=${jobId}`);
      }
    } catch (error: any) {
  ...
  const handlePaymentSelection = async () => {
    ...
    try {
      ...
      if (paymentMethod === 'card') {
        ...
        navigate(`/booking-success?jobId=${createdJobId}`); // durable confirmation
      }
    } catch (err: any) {
  ...
  const finalizeBooking = async (data: BookingFormValues, currentSnapshot: any, paymentId?: string) => {
    ...
    try {
      ...
      navigate(`/booking-success?jobId=${jobId}`); // durable confirmation
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-background pb-32 relative overflow-hidden">
      ...
      <div className="bg-charcoal text-white py-14 px-6 mb-12 relative overflow-hidden">
        ...
        {/* Subtle rock-art impression across the booking header (not over the form) */}
        ...
        <div className="cultural-values-strip absolute bottom-0 left-0" />
        ...
        <div className="max-w-xl mx-auto relative z-10">
          ...
          <div className="text-center">
            <div className="flex items-center justify-center gap-3">
              <img src="/cultural/rock-impression-figure-1.png" alt="" aria-hidden="true" className="h-9 w-auto opacity-50 hidden sm:block" />
              <h1 className="text-4xl font-black mb-2 tracking-tight italic uppercase">Book Your Mow</h1>
              <img src="/cultural/rock-impression-figure-2.png" alt="" aria-hidden="true" className="h-9 w-auto opacity-50 hidden sm:block" />
            </div>
              ...
              <CardContent className="space-y-4 pt-4 px-4 pb-6">
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder="e.g. 12 Simpson Street"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    <Label className="text-clay/50 font-black uppercase text-[8px] tracking-[0.2em] italic">Access notes / gate info (optional)</Label>
                    <Textarea
                      value={accessNotes}
                      onChange={(e) => setAccessNotes(e.target.value)}
                      className="text-xs rounded-xl min-h-[60px]"
                      placeholder="Gate code, dogs on site, parking, best entry..."
                    />
                  </div>

                  {locationConfirmed && (
                    <p className="text-[10px] font-black text-primary uppercase tracking-widest italic flex items-center gap-1">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!addr) { toast.error('Please enter your property address.'); return; }
                    if (!sub) { toast.error('Please enter your suburb.'); return; }
          ...
          {step === 6 && (
            <Card className="border-border shadow-premium rounded-[32px] overflow-hidden bg-surface/80 backdrop-blur-sm">
              ...
              <CardContent className="space-y-4 pt-6 px-4 pb-6 text-center">
                <p className="text-[10px] font-bold text-clay uppercase tracking-[0.2em] mb-4">Select Payment Method</p>
                <div className="grid grid-cols-2 gap-3">
                   <button 
                      type="button" 
