...
import { Locate, Search, Loader2, MapPin, Navigation, AlertCircle } from 'lucide-react';
...
export const LocationPicker: React.FC<LocationPickerProps> = ({ 
  ...
  // Graceful fallback: no API key, or the Maps API never finishes loading
  // (bad key / blocked referrer / API not enabled) → hide the broken map and
  // let the user type the address + confirm manually. Never breaks the page.
  const noMapsKey = !import.meta.env.VITE_GOOGLE_MAPS_PLATFORM_KEY;
  const [mapsTimedOut, setMapsTimedOut] = React.useState(false);
  const mapsUnavailable = noMapsKey || mapsTimedOut;

  // Map ID is OPTIONAL. Advanced Markers need a valid Map ID; if it's blank we
  // omit it (default raster map) and use a normal marker. Never fail on blank.
  const rawMapId = (import.meta.env.VITE_GOOGLE_MAPS_MAP_ID || '').trim();
  const mapId = rawMapId || undefined;
  const useAdvancedMarker = !!mapId;
  React.useEffect(() => {
    if (apiIsLoaded) { setMapsTimedOut(false); return; }
    const t = window.setTimeout(() => setMapsTimedOut(true), 8000);
  ...
  // Run auto-detect at most once (guards against React StrictMode double-invoke,
  // which otherwise fires the GPS flow + its warning twice).
  const autoDetectedRef = React.useRef(false);
  React.useEffect(() => {
    if (!apiIsLoaded || !geocodingLib || initialLocation) return;
    if (autoDetect && !autoDetectedRef.current) {
  ...
  const handleDetectLocation = () => {
    if (!navigator.geolocation) {
      toast.error('GPS not supported by your browser', { id: 'gps-unsupported' });
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        if (gpsAccuracy > 100) {
          ...
          setSource('gps');
          toast('Couldn’t get a precise GPS fix. Please type your address above or drag the pin.', { id: 'gps-accuracy', duration: 5000, icon: '📍' });
      ...
      (error) => {
        ...
        setIsDetecting(false);
        setSource('pin');
        toast.error('GPS detection failed. Use address search or move pin manually.', { id: 'gps-failed' });
    ...
    try {
      ...
      if (response.results[0]) {
        ...
        if (locType === 'APPROXIMATE' && accuracyVal > 50) {
          setIsLowAccuracy(true);
          toast.error('Address is vague. Please move pin to the exact property entrance.', { id: 'geocode-vague' });
      ...
      if (error?.message?.includes('REQUEST_DENIED') || error?.toString()?.includes('REQUEST_DENIED')) {
        toast('Address-from-map lookup is off. Please type and select your address above instead.', { id: 'geocode-denied', icon: '📍', duration: 5000 });
      } else {
        toast.error('Failed to resolve address. Please move pin or search.', { id: 'geocode-failed' });
      }
    } finally {
  ...
  const handleConfirmLocation = () => {
    if (isLowAccuracy && source === 'gps') {
      toast.error('Accuracy too low. Please search for specific address or adjust pin.', { id: 'confirm-accuracy' });
      return;
    }

    if (!address) {
      toast.error('Please wait for address sync or search for one.', { id: 'confirm-noaddress' });
      return;
    }
    ...
    toast.success('Location confirmed.', { id: 'location-confirmed' });
  ...
  return (
    <div className={cn("space-y-4", className)}>
        ...
        <div className="relative group">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              setSearchQuery(v);
              // Fallback: when predictive lookup (Google Places) isn't available,
              // let the typed text act as the address; the map pin supplies coordinates.
              if (!placesLib) {
                setAddress(v);
                setSource('pin');
                setIsConfirmed(false);
            ...
            placeholder={placesLib ? "Start typing your address for suggestions…" : "Type your property address…"}
            className={cn(
      ...
      <div className="relative h-[400px] rounded-2xl overflow-hidden border-2 border-border shadow-premium group/map">
        {mapsUnavailable ? (
          <div className="absolute inset-0 bg-stone-50 flex flex-col items-center justify-center p-6 text-center">
            <MapPin className="w-10 h-10 text-charcoal/30 mb-3" />
            <p className="text-xs text-charcoal/70 font-bold mt-1 max-w-xs leading-relaxed">
              Google address lookup is unavailable. You can still type the address manually.
            </p>
          </div>
        ) : !apiIsLoaded ? (
        ...
        <div className="absolute bottom-6 left-6 right-6 flex flex-col gap-3">
            ...
            <div className="flex-1 min-w-0">
               <div className="flex items-center gap-2 mb-1">
                <span className="text-[9px] font-black text-secondary uppercase tracking-[0.2em]">
                  {source === 'gps' ? `GPS location (${Math.round(accuracy || 0)}m)` : source === 'places' ? 'Verified address' : 'Manual pin'}
                </span>
              ...
              <p className={cn(
                ...
                {isDetecting ? "Finding your location…" :
                 isGeocoding ? "Checking address…" :
                 isLowAccuracy ? "Low accuracy — adjust the pin" :
                 address || (mapsUnavailable ? "Type your address above to continue" : "Searching for address…")}
              </p>
            </div>
          </div>
          ...
          {!isConfirmed && !isDetecting && !isGeocoding && address && (
            ...
            >
              {isLowAccuracy ? "Low accuracy — adjust the pin" : "Confirm Location"}
            </Button>
          )}
        </div>
        ...
        {placesError && (
          <div className="absolute inset-0 bg-red-50/90 backdrop-blur-sm flex flex-col items-center justify-center p-8 text-center z-50">
             ...
             <h3 className="text-lg font-black text-charcoal uppercase tracking-tighter mb-2">Address search unavailable</h3>
