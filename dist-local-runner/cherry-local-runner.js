...
function isAllowedWriteFile(filePath) {
...
function createBackup(files) {
    ...
    fs.writeFileSync(path.join(backupFolder, "backup-report.json"), JSON.stringify(report, null, 2), "utf8");
    return {
        backupFolderName,
        backupFolder,
        copied,
...
function extractUsefulErrors(logText) {
    return logText
        .split(/\r?\n/)
        .filter((line) => /error TS|ERROR|Error:|Cannot|failed|not found|npm ERR|Build failed|SyntaxError|TypeError/i.test(line))
        .slice(0, 80);
}
...
app.get("/project/status", (_req, res) => {
    const files = [
        "package.json",
        "Dockerfile",
        "cherry-server.ts",
...
app.get("/logs/list", (_req, res) => {
    try {
        const logFiles = fs
            .readdirSync(PROJECT_ROOT)
            .filter((file) => /log|report|diagnose|error|build|deploy/i.test(file))
...
app.post("/deploy/run", async (req, res) => {
    try {
        const approvalCode = String(req.body?.approvalCode || "");
        ...
        const startedAt = new Date().toISOString();
        const gcloudPath = '"C:\\Program Files (x86)\\Google\\Cloud SDK\\google-cloud-sdk\\bin\\gcloud.cmd"';
        const command = `${gcloudPath} run deploy cherry-core --source . --region australia-southeast1 --allow-unauthenticated`;
...
app.get("/tools", (_req, res) => {
    res.json({
        ...
        safety: "Cherry can now run npm run build only when David build approval code is supplied. Build output is saved to CHERRY_BUILD_RUNNER_LOG.txt.",
    });
});
