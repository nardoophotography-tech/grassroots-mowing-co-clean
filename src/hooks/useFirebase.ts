...
import { UserProfile, Job, Client, Invoice, BusinessSettings, InvoiceItem, AccountStatus, PaymentMethod, PricingRules, Payment } from '../types';
...
export function useJobs() {
  ...
  const updateJob = async (id: string, jobData: Partial<Job>) => {
    ...
    try {
      ...
      if (jobData.status === 'on-the-way') {
        ...
        await triggerNotification('completed', mergedJob);
  ...
  const sendQuoteToCustomer = async (jobId: string) => {
    ...
    try {
      ...
      await triggerNotification('quote-sent', { ...job, quoteUrl });
  ...
  const generateInvoiceForJob = async (jobId: string) => {
    ...
    await triggerNotification('invoice-sent', job, { amount: job.price, link: paymentLink });
...
export function useInvoices() {
  ...
  const markAsPaid = async (invoiceId: string, method: PaymentMethod) => {
    ...
    if (jobSnap.exists()) {
      await triggerNotification('payment-receipt', { id: jobSnap.id, ...jobSnap.data() } as Job, { amount: inv.totalAmount });
    }
...
export function useClients() {
  ...
  const addClient = async (clientData: Omit<Client, 'id' | 'createdAt' | 'accountStatus'>) => {
    const path = 'clients';
    try {
      const docRef = await mythosAddDoc(collection(db, path), {
        ...clientData,
