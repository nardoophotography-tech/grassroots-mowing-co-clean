...
export const PasscodeModal: React.FC<PasscodeModalProps> = ({
  ...
  // OWNER RECOVERY FIX — PIN/PASSCODE MODAL DISABLED.
  // Normal email/admin login should be used instead.
  return null;
  ...
  // Disables passcode modal entirely on localhost / 127.0.0.1
  const _isLocalDev = ['localhost', '127.0.0.1'].includes(window.location.hostname);
  if (_isLocalDev || !isOpen) return null;
