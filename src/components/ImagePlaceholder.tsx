...
export const ImagePlaceholder: React.FC<ImagePlaceholderProps> = ({ 
  ...
  const imageUrl = src || `https://picsum.photos/seed/${seed}/${width}/${height}`;

  return (
    <div 
      className={cn(
        "relative group overflow-hidden rounded-xl bg-charcoal/5 border-2 border-dashed border-ochre/20 flex items-center justify-center transition-all duration-500 hover:border-ochre/40",
        className
      )}
      style={{ aspectRatio }}
    >
      <img
        ...
        className={cn(
          ...
          !src && "opacity-60 group-hover:opacity-80"
        )}
      />
