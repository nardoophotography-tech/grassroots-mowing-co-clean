...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // LOCAL DEV ONLY — bypass lock via ?localUnlock=true on localhost/127.0.0.1
  React.useEffect(() => {
    const isLocal = ['localhost', '127.0.0.1'].includes(window.location.hostname);
    if (isLocal && new URLSearchParams(window.location.search).get('localUnlock') === 'true') {
  ...
  const logout = async () => {
    try {
      ...
      setProfile(null);
      setIsLocked(false);
      window.location.replace('/login?intendedRole=admin');
    } catch (error) {
