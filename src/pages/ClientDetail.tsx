...
import { useClients, useJobs, useInvoices, useSettings } from '@/src/hooks/useFirebase';
...
import { ACCOUNT_STATUS_LABELS, JOB_STATUS_LABELS, TIME_SLOT_LABELS } from '@/src/constants';
            ...
            <CardContent className="pt-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-deep-red/[0.03] rounded-xl border border-deep-red/10">
                  <p className="text-[8px] text-deep-red font-black uppercase tracking-[0.2em] mb-1">Pending</p>
                  <p className="text-2xl font-black text-charcoal">${pendingAmount.toFixed(2)}</p>
                </div>
              </div>
            ...
            <Card className="border-ochre/20 shadow-xl rounded-xl overflow-hidden bg-white">
              ...
              <CardContent className="pt-6">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-ochre pl-1">Address</Label>
                      <Input {...register('address')} className="h-12 rounded-xl border-ochre/20" />
                      {errors.address && <p className="text-xs text-red-500 mt-1">{errors.address.message}</p>}
                    </div>
                    <div className="space-y-2">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-ochre pl-1">Suburb</Label>
                      <Select {...register('suburb')} className="h-12 rounded-xl border-ochre/20">
                        <option value="">Select Region</option>
                        {(settings?.suburbSchedules.map(s => s.suburb) || []).map(s => (
