...
import { JOB_STATUS_LABELS, JOB_STATUS_COLORS, TIME_SLOT_LABELS, ADD_ON_LABELS } from '@/src/constants';
...
import { useInvoices, useAdmin, useJobs as useJobsHook } from '@/src/hooks/useFirebase';
import { PaymentMethod, AddOn, JobIssue } from '@/src/types';
import { useAuth } from '@/src/contexts/AuthContext';
import { PasscodeModal } from '@/src/components/PasscodeModal';
import { PRICING_RULES } from '@/src/constants';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/src/components/ui/Tabs';
import { InvoiceDownloadButton } from '@/src/components/InvoiceDownloadButton';
        ...
        <div className="md:col-span-2 space-y-6">
          <Card className="border-ochre/10 shadow-lg rounded-xl overflow-hidden">
            ...
            <CardContent className="space-y-4 pt-6">
              <p className="text-sm text-charcoal font-bold">{job.address}, {job.suburb}</p>
            </CardContent>
          </Card>
              ...
              <CardHeader className="bg-deep-red text-white">
                ...
                <div className="mt-4">
                  <InvoiceDownloadButton 
                    ...
                    variant="ghost"
                    className="text-white hover:bg-white/10 border-white/20 w-full"
                  />
                </div>
              </CardHeader>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-ochre/5">
                    {invoice.items.map((item, i) => (
                      <tr key={i} className="bg-white">
                        ...
                        <td className="p-4 text-right font-black text-charcoal">${item.amount.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot className="bg-ochre/5 font-black">
                    <tr>
                      ...
                      <td className="p-4 text-right text-deep-red text-2xl font-black">${invoice.totalAmount.toFixed(2)}</td>
                    </tr>
                  </tfoot>
                </table>
                {invoice.status === 'paid' && (
                  <div className="p-4 bg-ochre/10 text-ochre text-[10px] font-bold uppercase tracking-widest flex justify-between items-center">
                    ...
                    <Button variant="ghost" size="sm" className="h-auto p-1 text-deep-red hover:bg-transparent font-black">Resend Receipt</Button>
                  </div>
                )}
              </CardContent>
            </Card>
              ...
              {job.pricingSnapshot ? (
                <>
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-black text-charcoal">${job.pricingSnapshot.basePrice.toFixed(2)}</span>
                  </div>
                  {job.pricingSnapshot.tierAdjustment !== 0 && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-black text-charcoal">{job.pricingSnapshot.tierAdjustment > 0 ? '+' : ''}${job.pricingSnapshot.tierAdjustment.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm">
                    <span className="text-ochre font-bold uppercase tracking-widest text-[10px]">Grade Adjustment ({job.serviceGrade})</span>
                    <span className="font-black text-charcoal">+${job.pricingSnapshot.gradeAdjustment.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-ochre font-bold uppercase tracking-widest text-[10px]">Condition Surcharges</span>
                    <span className="font-black text-charcoal">+${job.pricingSnapshot.conditionSurcharge.toFixed(2)}</span>
                  </div>
                  {job.pricingSnapshot.urgencySurcharge > 0 && (
                    <div className="flex justify-between text-sm text-deep-red">
                      ...
                      <span className="font-black">+${job.pricingSnapshot.urgencySurcharge.toFixed(2)}</span>
                    </div>
                  )}
                ...
                <>
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-black text-charcoal">${job.basePrice.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-ochre font-bold uppercase tracking-widest text-[10px]">Grade Adjustment ({job.serviceGrade})</span>
                    <span className="font-black text-charcoal">+${job.gradeAdjustment.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-ochre font-bold uppercase tracking-widest text-[10px]">Condition Surcharges</span>
                    <span className="font-black text-charcoal">+${job.conditionSurcharge.toFixed(2)}</span>
                  </div>
                  {job.urgencySurcharge > 0 && (
                    <div className="flex justify-between text-sm text-deep-red">
                      <span className="font-bold uppercase tracking-widest text-[10px]">Urgency Surcharge</span>
                      <span className="font-black">+${job.urgencySurcharge.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm">
                    <span className="text-ochre font-bold uppercase tracking-widest text-[10px]">Add-Ons Total</span>
                    <span className="font-black text-charcoal">+${job.addOnTotal.toFixed(2)}</span>
                  </div>
                </>
              )}

              <div className="pt-4 border-t border-ochre/20 flex gap-4 mt-2">
                <Button 
                  variant="outline" 
                  size="sm"
                  onClick={() => sendQuoteToCustomer(job.id)}
          ...
          <Card className="border-ochre/10 shadow-lg rounded-xl overflow-hidden">
            ...
            <CardContent className="space-y-3 pt-6">
              {job.status === 'scheduled' && (
                <Button className="w-full bg-deep-red hover:bg-deep-red/90 text-white h-12 rounded-xl font-bold shadow-lg" onClick={() => handleStatusChange('on-the-way')} isLoading={isUpdating}>
                  <Truck className="mr-2 h-4 w-4" /> On The Way
                </Button>
              )}
              {job.status === 'on-the-way' && (
                <Button className="w-full bg-deep-red hover:bg-deep-red/90 text-white h-12 rounded-xl font-bold shadow-lg" onClick={() => handleStatusChange('in-progress')} isLoading={isUpdating}>
                  <Play className="mr-2 h-4 w-4" /> Start Job
                </Button>
              )}
              {job.status === 'in-progress' && (
                <Button className="w-full bg-deep-red hover:bg-deep-red/90 text-white h-12 rounded-xl font-bold shadow-lg" onClick={() => handleStatusChange('completed')} isLoading={isUpdating}>
                  <CheckCircle className="mr-2 h-4 w-4" /> Complete & Invoice
                </Button>
              )}
              {job.status === 'invoiced' && (
                ...
                <ul className="space-y-3">
                  {job.addOns.filter(a => a.selected).map(addon => (
                    <li key={addon.id} className="text-sm flex justify-between items-center">
                      ...
                      <span className="font-black text-deep-red">${addon.price.toFixed(2)}</span>
                    </li>
                  ))}
                </ul>
              </CardContent>
