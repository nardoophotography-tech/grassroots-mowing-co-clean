...
import { useClients, useSettings } from '@/src/hooks/useFirebase';
...
import { ACCOUNT_STATUS_LABELS, SUBURBS } from '@/src/constants';
  ...
  return (
    <div className="p-4 lg:p-8 space-y-8 relative overflow-hidden pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
        <div>
          <h2 className="text-3xl font-black text-secondary uppercase italic tracking-tight underline decoration-clay/30 underline-offset-8">Client Database</h2>
          ...
          <CardContent className="pt-8">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  <Label htmlFor="address" className="text-charcoal font-black uppercase text-[10px] tracking-widest pl-1">Svc. Address</Label>
                  <Input id="address" {...register('address')} placeholder="123 Homestead Ave" className="h-12 rounded-xl border-ochre/20" />
                  {errors.address && <p className="text-xs text-red-500 font-bold italic">{errors.address.message}</p>}
                </div>

                <div className="space-y-3">
                  <Label htmlFor="suburb" className="text-charcoal font-black uppercase text-[10px] tracking-widest pl-1">Target Suburb</Label>
                  <Select id="suburb" {...register('suburb')} className="h-12 rounded-xl border-ochre/20">
                    <option value="">Select Region</option>
                    {(settings?.suburbSchedules.map(s => s.suburb) || SUBURBS).map(s => (
                      <option key={s} value={s}>{s}</option>
                    ))}
                  </Select>
                  {errors.suburb && <p className="text-xs text-red-500 font-bold italic">{errors.suburb.message}</p>}
                </div>
              </div>
