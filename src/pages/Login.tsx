...
export const Login = () => {
  ...
  React.useEffect(() => {
    const handleRoleRedirect = async () => {
      if (user) {
        ...
        // If they are authenticating for onboarding, send them back to the onboarding page
  ...
  const currentTheme = (intendedRole && themeVariants[intendedRole as keyof typeof themeVariants]) || themeVariants.default;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Safety check: ensure values are strings and not empty
    ...
    <div className={`min-h-screen flex flex-col items-center justify-center ${currentTheme.bg} p-4 relative overflow-hidden transition-colors duration-500`}>
      <div className="absolute top-4 left-4 flex gap-2 z-20">
        <Button
          ...
          className={`${currentTheme.text} hover:bg-black/5 h-10 w-10 rounded-full border border-black/10 p-0 flex items-center justify-center`}
          title="Go Back"
        >
        ...
        <CardContent className="pb-10 pt-2">
          {unauthorized ? (
            <div className="space-y-6">
              <div className={`p-6 rounded-[30px] ${intendedRole === 'client' ? 'bg-stone-800 border-stone-700' : 'bg-amber-50 border-amber-200'} border text-center`}>
                <div className={`w-12 h-12 ${intendedRole === 'client' ? 'bg-stone-700' : 'bg-amber-100'} rounded-full flex items-center justify-center mx-auto mb-4`}>
                  <ShieldCheck className={`h-6 w-6 ${intendedRole === 'client' ? 'text-stone-300' : 'text-amber-600'}`} />
                </div>
                <h3 className={`font-bold ${intendedRole === 'client' ? 'text-white' : 'text-amber-900'} mb-2`}>Entry Restricted</h3>
                <p className={`text-xs ${intendedRole === 'client' ? 'text-stone-400' : 'text-amber-800'} leading-relaxed`}>
                  Authentication successful, but you are not registered in the <strong>{currentTheme.heading}</strong> {intendedRole === 'client' ? 'service' : 'workforce'} database.
                </p>
