  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await notificationService.notifyRole(
        'admin', 
        'Quote Accepted', 
        `Client has approved quote for ${job.location?.address || job.address}. Total: $${job.price}.`,
        `/jobs/${jobId}`,
        'success'
      );
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      await notificationService.notifyRole(
        'admin', 
        'Quote Rejected', 
        `Client has rejected the quote for ${job.location?.address || job.address}.`,
        `/jobs/${jobId}`,
        'error'
      );
  ...
  return (
    <div className="min-h-screen bg-cream pb-12 relative overflow-hidden">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 p-3 bg-white border border-ochre/10 rounded-xl">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-charcoal/50 leading-tight">Professional mow with sharp edges</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 p-3 bg-white border border-ochre/10 rounded-xl">
                      ...
                      <div>
                        <p className="font-bold text-sm text-charcoal capitalize">{job.serviceGrade} Condition</p>
                        <p className="text-[10px] text-charcoal/50 leading-tight">Based on your yard's height and thickness</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="lg:col-span-1">
            <Card className="border-ochre/20 shadow-2xl sticky top-8 overflow-hidden">
              <CardHeader className="bg-charcoal text-white border-b border-ochre/10">
                <CardTitle className="text-lg font-serif">Job Quote</CardTitle>
                <p className="text-[10px] font-bold text-ochre/80 uppercase tracking-[0.2em] mt-1">Verified Pricing</p>
              </CardHeader>
              <CardContent className="pt-6 space-y-4">
                {snapshot ? (
                  <div className="space-y-3">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-bold text-charcoal">${(snapshot.basePrice || 0).toFixed(2)}</span>
                    </div>
                    {snapshot.tierAdjustment !== 0 && (
                      <div className="flex justify-between text-xs italic">
                        ...
                        <span className={cn("font-bold", snapshot.tierAdjustment > 0 ? "text-deep-red" : "text-green-600")}>
                          {snapshot.tierAdjustment > 0 ? '+' : ''}${(snapshot.tierAdjustment || 0).toFixed(2)}
                        </span>
                      </div>
                    )}
                    {(snapshot.gradeAdjustment !== 0 || snapshot.conditionSurcharge !== 0 || snapshot.urgencySurcharge !== 0) && (
                      <div className="flex justify-between text-xs">
                        <span className="text-charcoal/60">Extra Costs (Grass/Condition)</span>
                        <span className="font-bold text-deep-red">+${((snapshot.gradeAdjustment || 0) + (snapshot.conditionSurcharge || 0) + (snapshot.urgencySurcharge || 0)).toFixed(2)}</span>
                      </div>
                    )}
                  ...
                  <div className="text-center py-4">
                    <p className="text-sm font-black text-deep-red uppercase tracking-widest">Total: ${(job.price || 0).toFixed(2)}</p>
                  </div>
                )}
                ...
                {job.quotePdfUrl && (
                  <Button 
                    variant="outline"
                    className="w-full h-10 border-ochre/20 text-ochre hover:bg-ochre/5 font-bold rounded-xl transition-all flex items-center justify-center gap-3 mt-4 mb-2"
                ...
                {job.quoteStatus === 'approved' ? (
                  ...
                  <div className="space-y-4 pt-4">
                    ...
                    >
                      ✔ Accept Quote & {paymentMethod === 'card' ? 'Pay Online' : 'Pay in Cash'}
                    </Button>
                    <Button 
                      variant="outline" 
      ...
      <div className="max-w-4xl mx-auto px-4 mt-12 text-center text-charcoal/20">
         ...
         <p className="text-[10px] uppercase tracking-widest font-bold mt-1 text-charcoal/40">A Project #156 Initiative</p>
      </div>
    </div>
  );
};
